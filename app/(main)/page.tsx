    ...
    <main className="w-full h-screen relative">
      {!isOpenGift ? (
        <div className="absolute bg-black/100 w-full h-full top-0 left-0 flex flex-col items-center justify-center gap-y-[60px] px-5">
          ...
          />
          <p className="font-semibold text-md text-white text-center w-full">
            Nhấn vào để nhận quà đi chứ còn đợi gì nữa? 😂
          </p>
        </div>
      ) : (
        <div className="absolute w-full h-full top-0 left-0">
            ...
            <AlertDialogContent className="w-[90%] m-auto rounded-lg max-w-[400px]">
              <AlertDialogHeader className="w-full flex flex-col justify-center items-center gap-y-[20px]">
                ...
                <Input
                  ...
                  placeholder=""
                  className="w-full focus-visible:ring-0 focus-visible:ring-offset-0 max-w-[300px]"
                />
              </AlertDialogHeader>
              <AlertDialogFooter>
          ...
          <AlertDialog open={openNameConfirm} onOpenChange={setOpenNameConfirm}>
            <AlertDialogContent className="w-[90%] m-auto rounded-lg max-w-[400px]">
              <AlertDialogHeader className="w-full flex flex-col justify-center items-center gap-y-[20px]">
                <AlertDialogTitle>
                  Tên của mình cũng không nhớ là sao nhỉ? Nhập &quot;Công Chúa
                  Uyển Nhi&quot; vào nhé! 😈
                </AlertDialogTitle>
              </AlertDialogHeader>
              <AlertDialogFooter>
      ...
      <video
        src="/video/backround_heart_new.webm"
        autoPlay
        loop
        muted
        preload="auto"
        className={`absolute top-0 left-0 w-full h-full object-cover z-[-1] ${
          isOpenGift ? "block" : "hidden"
        }`}
      />
      {showContent && (
