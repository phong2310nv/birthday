  ...
  return (
    <>
      ...
      {finishFirst && !finishSecond && (
        <AnimatedText
          text="Đợi đã 😗😗😗 đợi 1 chút này 😗😗😗 nay là ngày gì ấy nhờ"
          delay={1}
          end={1000}
          setFinish={setFinishSecond}
        />
      )}
      {finishFirst && finishSecond && (
        <div className="flex flex-col items-start justify-center text-center gap-y-4">
          ...
          >
            Happy Women&apos;s day to my love Công Chúa Uyển Nhi 🥳
          </motion.p>
          {!endContentOne && (
            <AnimatedText
              text="8/3 thì người ta hay tặng hoa nè, nhưng bông hoa đẹp nhất trong cuộc đời anh chính là em"
              delay={1}
              end={500}
              setFinish={setFinishThird}
            />
          )}
          {finishThird && (
            <AnimatedText
              text="Chúc em, bông hoa đẹp nhất của đời a luôn mãi mãi xinh đẹp, đáng yêu và gặp nhiều niềm vui nhé."
              delay={1}
              end={500}
              setFinish={setFinishFourth}
            />
          )}
          {finishThird && finishFourth && (
            <AnimatedText
              text="Tuy giờ,  em, anh và 2 đứa cũng đã trải qua nhiều niềm vui nỗi buồn cùng nhau. 2 đứa có nhiều tâm tư nguyện vọng cho nhau nhưng chưa nói hay chưa làm được. Nhưng hãy vẫn thương yêu nhau như ngày mà chúng ta đến với nhau nhé. Dù bất kể có j khó khăn hãy nắm tay a và mình cùng vượt qua nó nha bé iu."
              delay={1}
              end={1000}
              setFinish={setFinishFifth}
            />
          )}
          {!lastContent && endContentOne && (
            <>
              <AnimatedText
                text="Tình yêu là sự bao dung, vị tha, chia sẻ và cam chịu sát cánh bên nhau mà. Anh biết em rất tự lập và không muốn giải thích quá nhiều 
Nhưng em giờ có anh ở bên, lắng nghe và đi cùng em bất kể em muốn đi đâu"
                delay={1}
                end={500}
                setFinish={setLastContent}
              />
          ...
          {lastContent && (
            <div
              className={`${dancingScript.className} text-center text-lg w-full`}
            >
              <AnimatedText
                text="Chúc mừng 8/3 nha công chúa của a 🥰 Anh yêu em, nhớ em nhiều lắm. Tý ra gặp anh đấy a còn bất ngờ nữa cho em cơ. 🥳🎉"
                delay={1}
                end={1000}
                setFinish={setLastContentQoute}
              />
      ...
      {finishFirst && finishThird && finishFifth && (
        <div
          className="mt-[20px] w-full flex justify-end"
      ...
      <AlertDialog open={showContent} onOpenChange={setShowContent}>
        <AlertDialogContent className="w-[90%] m-auto rounded-lg max-w-[400px]">
          <AlertDialogHeader className="w-full flex flex-col justify-center items-center gap-y-[20px]">
            ...
            />
            <AlertDialogTitle>
              Công Chúa Uyển Nhi có muốn nhận quà tiếp không nhỉ? 😁😘
            </AlertDialogTitle>
      ...
      <AlertDialog open={showContentSecond} onOpenChange={setShowContentSecond}>
        <AlertDialogContent className="w-[90%] m-auto rounded-lg max-w-[440px] py-[40px]">
          <AlertDialogHeader className="w-full flex flex-col justify-center items-center gap-y-[20px]">
            ...
            />
            <AlertDialogTitle className="text-md">
              Thôi đừng dỗi nữa, quà ở đây nè!😄
            </AlertDialogTitle>
          </AlertDialogHeader>
        </AlertDialogContent>
      </AlertDialog>
